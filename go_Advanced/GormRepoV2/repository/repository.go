...
type Repository interface {
	Get(uow *UnitOfWork, out interface{}, id int, preloadAssociations []string) error
	GetAll(uow *UnitOfWork, out interface{}, preloadAssociations []string) error
	GetAllForTenant(uow *UnitOfWork, out interface{}, tenantID int, preloadAssociations []string) error
...
func (repository *GormRepository) Get(uow *UnitOfWork, out interface{}, id int, preloadAssociations []string) error {
	db := uow.DB
...
func (repository *GormRepository) GetAllForTenant(uow *UnitOfWork, out interface{}, tenantID int, preloadAssociations []string) error {
	db := uow.DB
	for _, association := range preloadAssociations {
		db = db.Preload(association)
	}
